...
def main():
    ...
    # --- Load Data ---
    data = pd.read_excel(r"D:\NIK'S_FOLDER\Python_with_Excel (Project-1)\property_sales_dataset.xlsx")
    st.write(data)
    ...
    if "Yearly Sales" in data.columns:
        data["Yearly Sales"] = pd.to_datetime(data["Yearly Sales"], errors="coerce")  # Convert to datetime
        data["Yearly Sales"] = data["Yearly Sales"].dt.strftime("%Y-%m")  # Convert to YYYY-MM format
